...
function Profile(){
  ...
  const fetchUserPosts = async () => {
    try {
      const response = await fetch('https://www.uhitch.live:5000/api/post/'+user.email, {
        method: 'GET',
        headers: {
          'Content-Type': 'application/json',
  ...
  const fetchAcceptedPosts = async () => {
    try {
      const response = await fetch('https://www.uhitch.live:5000/api/post/accept/'+user.email, {
        method: 'GET',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${user.token}`, // Adding the Authorization header with the token
...
const handleClick = async () => {
    ...
    try {
      ...
      const response = await fetch(`https://www.uhitch.live:5000/api/user/sendData/${id}`, {
        method: 'PATCH',
        headers: {
          'Content-Type': 'application/json',
        },
